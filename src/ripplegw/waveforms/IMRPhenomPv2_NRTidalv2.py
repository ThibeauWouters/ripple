...
from .IMRPhenomD_NRTidalv2 import gen_IMRPhenomD_NRTidalv2
...
def gen_IMRPhenomPv2_NRTidalv2(
    ...
    """
    m1, m2, s1x, s1y, s1z, s2x, s2y, s2z, lambda1, lambda2, dist_mpc, tc, phiRef, incl = theta
    ...
    theta_intrinsic_bbh = jnp.array([m2, m1, chi2_l, chi1_l])
    coeffs = get_coeffs(theta_intrinsic_bbh)
    
    mc, eta = ms_to_Mc_eta(jnp.array([m1, m2]))
    params_IMRPhenomD_NRTidalv2 = jnp.array([mc, eta, chi2_l, chi1_l, lambda2, lambda1, dist_mpc, tc, phiRef])

    # TODO: check if this is still the same for NRTv2?
    transition_freqs = phP_get_transition_frequencies(
        theta_intrinsic_bbh, coeffs[5], coeffs[6], chip
    )

    phi_IIb = lambda f: -PhDPhase(f, theta_intrinsic_bbh, coeffs, transition_freqs)

    hPhenomDs = gen_IMRPhenomD_NRTidalv2(
        fs, params_IMRPhenomD_NRTidalv2, f_ref, use_lambda_tildes=use_lambda_tildes
    )
    ...
    # phase_corr = jnp.c    os(2 * jnp.pi * fs * (t0)) - 1j * jnp.sin(2 * jnp.pi * fs * (t0))
    # M_s = (m1 + m2) * gt
...
def gen_IMRPhenomPv2_NRTidalv2_hphc(f: Array, params: Array, f_ref: float):
    """
    ...
    hp, hc = gen_IMRPhenomPv2_NRTidalv2(f, m1m2params, f_ref)
    return hp, hc
